...
use core::{sync::atomic::AtomicUsize, cell::SyncUnsafeCell, alloc::Layout, ptr::null_mut};
...
static mut FREE_BLOCK_STORAGE: heapless::Vec<(usize, usize), FREE_BLOCKS_SIZE> = heapless::Vec::<(usize, usize), FREE_BLOCKS_SIZE>::new();

impl KernelAllocator{
    const unsafe fn new() -> Self{
        KernelAllocator { arena: SyncUnsafeCell::<[u8; ARENA_SIZE]>::new([0x0; ARENA_SIZE]), is_initialized: AtomicBool::new(false), size: AtomicUsize::new(ARENA_SIZE), remaining: AtomicUsize::new(ARENA_SIZE) }
    }
...
unsafe impl Sync for KernelAllocator { /* empty */ }
